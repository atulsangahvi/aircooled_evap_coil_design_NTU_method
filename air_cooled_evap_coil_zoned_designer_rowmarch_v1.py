...
# ---------------- Geometry / Areas ----------------
def geometry_areas(face_W, face_H, Nr, St, Do, tf, FPI):
    face_area = face_W*face_H
    s = (1.0/FPI)*INCH
    fins = max(int(round(face_H / max(s,1e-9))), 1)
    tubes_per_row = max(int(math.floor(face_H / max(St,1e-9))), 1)
    N_tubes = tubes_per_row * Nr
    L_tube = face_W

    # fin 2-face area minus holes
    A_holes_one_fin = N_tubes * (pi*(Do/2.0)**2)
    A_fin_one = max(2.0*(face_W*face_H - A_holes_one_fin), 0.0)
    A_fin_total = A_fin_one * fins

    # bare tube exposure between fins
    ...
    tube_blockage = min(A_holes_one_fin/max(face_W*face_H,1e-9), 0.5)
    Amin = max(face_area*(1.0 - fin_blockage - tube_blockage), 1e-4)

    return dict(face_area=face_area, s=s, fins=fins, tubes_per_row=tubes_per_row,
                N_tubes=N_tubes, L_tube=L_tube, A_fin=A_fin_total, A_bare=A_bare,
...
def simulate_evaporator(
    ...
    # Geometry
    geom = geometry_areas(face_W, face_H, Nr, St, Do, tf, FPI)
    ...
    h_air_dry, air_meta = air_htc_zukauskas(rho_in, mu_a, k_a, Pr_a, Do, Nr, mdot_air_total, Amin)
        ...
        if x is None:
            ...
            if Q_row_max >= Q_to_Tsat and Q_to_Tsat > 0:
                ...
                rows_log.append(dict(
                    ...
                    Q_row_kW=(Q_sh)/1000.0, T_air_out=T_air, W_air_out=W_air,
                    T_ref=T_ref, x=x, frac_used=frac, Re_i=Re_i, v_ref=v_i
                ))
            else:
                ...
                rows_log.append(dict(
                    ...
                    Q_row_kW=Q/1000.0, T_air_out=T_air, W_air_out=W_air,
